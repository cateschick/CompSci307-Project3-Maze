...
public class MazeDisplay {
	...
	private static SearchAlgorithm mySolver;
	...
	public Scene setupDisplay (Paint background) {
		...
		myScene = new Scene(root, background);
	...
	/**
	 * This method adds algorithm buttons to the algorithm panel
	 * @param description of which algorithm is being used
	 * @param panel
	 */
	private void addButton(String description, HBox panel) {
		Button b = new Button(description);
		switch (description) {
			case "Breadth-First Search" -> b.setOnAction(value ->  setSearch(new BFS(myMaze)));
			case "Depth-First Search" -> b.setOnAction(value ->  setSearch(new DFS(myMaze)));
			case "Greedy Search" -> b.setOnAction(value ->  setSearch(new Greedy(myMaze)));
			case "Random Walk Search" -> b.setOnAction(value ->  setSearch(new RandomWalk(myMaze)));
			case "Magic Search" -> b.setOnAction(value ->  setSearch(new Magic(myMaze)));
			case "New Maze" -> b.setOnAction(value -> newMaze());
			case "Pause" -> b.setOnAction(value ->  togglePause());
		}
		panel.getChildren().add(b);
	}

	private Node updateSteps() {
		Label panel = new Label();
		if (mySolver != null) {
			panel.setText("STEPS TAKEN: " + mySteps);
		} else {
			panel.setText("STEPS TAKEN: 0");
		}

		panel.setFont(new Font("Arial", 20));
		return panel;
	}
	...
	private Node makeAlgorithmPanel () {
		...
		// Add buttons
		addButton("Breadth-First Search", panel);
		addButton("Depth-First Search", panel);
		addButton("Greedy Search", panel);
		addButton("Magic Search", panel);
		addButton("Random Walk Search", panel);
	...
	private Node makeControlPanel () {
		...
		// Add buttons
		addButton("New Maze", control);
		addButton("Pause", control);
	...
	private void step () {
		mySteps = mySolver.getSteps();
		mySize = mySolver.getMax();
		myBacktracks = mySolver.getBacktracks();

		// TODO: Have this be displayed on the front end
		System.out.println("STEPS TAKEN: " + mySteps + ", " + "MAX SIZE: " + mySize + ", " + "BACKTRACKS: " + myBacktracks);

		if (mySolver != null) {
			if (mySolver.step()) {
				// CHANGE BACKGROUND ONLY ON SUCCESSFUL SEARCH
				if (mySolver.successfulSearch()) {
					root.setBackground(new Background(new BackgroundFill(Color.LIMEGREEN, CornerRadii.EMPTY, Insets.EMPTY)));
				}
				else {
	...
	private void togglePause() {
		Button b = (Button) control.getChildren().get(1);
		if (isPaused) {
			b.setText("Resume");
			myAnimation.play();
		}
